...
import { MAX_PAYLOAD_BYTES, MAX_QUEUE_DEPTH_PER_DEVICE, RATE_LIMIT_MAX_MESSAGES } from './limits.js';

async function openAuthorizedSocket(room: string, device: string, token: string): Promise<WebSocket> {
  const res = await SELF.fetch(`https://example.com/room/${room}?device=${device}&token=${token}`, {
...
async function registerDevice(
  ...
  const consumeRes = await SELF.fetch(`https://example.com/room/${room}/pair/consume`, {
    method: 'POST',
...
describe('per-recipient routing', () => {
  it('delivers a message addressed to one device only to that device', async () => {
    ...
    const tokenB = await registerDevice(room, 'device-b', { asDevice: 'device-a', token: tokenA });
    const tokenC = await registerDevice(room, 'device-c', { asDevice: 'device-a', token: tokenA });
    ...
    a.send(JSON.stringify({ v: 1, room, device: 'device-a', to: 'device-b', kind: 'handoff', iv: 'x', ciphertext: 'y', ts: 1 }));
  ...
  it('delivers a "to: all" message to every other connected device', async () => {
    ...
    const bReceived = nextMessage(b);
    a.send(JSON.stringify({ v: 1, room, device: 'device-a', to: 'all', kind: 'presence', iv: 'x', ciphertext: 'y', ts: 1 }));
    const message = await bReceived;
  ...
  it('drops an envelope missing `to` instead of broadcasting to every socket', async () => {
    ...
    a.send(JSON.stringify({ v: 1, room, device: 'device-a', kind: 'handoff', iv: 'x', ciphertext: 'y', ts: 1 }));
    await new Promise((resolve) => setTimeout(resolve, 50));
...
describe('pairing', () => {
  ...
  it('lets a second device consume the code for its own token', async () => {
    ...
    const issueRes = await SELF.fetch(`https://example.com/room/${room}/pair/issue`, {
      method: 'POST',
      body: JSON.stringify({ device: 'device-a' }),
    });
  ...
  it('rejects reusing a pairing code', async () => {
    ...
    const secondAttempt = await SELF.fetch(`https://example.com/room/${room}/pair/consume`, {
      method: 'POST',
  ...
  it('rejects an expired pairing code', async () => {
    ...
    await runInDurableObject(stub, async (_instance: unknown, state: DurableObjectState) => {
      state.storage.sql.exec('UPDATE pairing_codes SET expires_at = ? WHERE code = ?', Date.now() - 1_000, code);
    });
  ...
  it('allows a non-bootstrap pair/issue with a valid token', async () => {
    ...
    const secondIssue = await SELF.fetch(`https://example.com/room/${room}/pair/issue`, {
      method: 'POST',
...
describe('device revoke', () => {
  it('revokes a device so its token no longer authorizes pair/issue', async () => {
    ...
    const revokeRes = await SELF.fetch(`https://example.com/room/${room}/pair/revoke`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${deviceToken}` },
      body: JSON.stringify({ device: 'device-a' }),
    });
    ...
    const reissueRes = await SELF.fetch(`https://example.com/room/${room}/pair/issue`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${deviceToken}` },
      body: JSON.stringify({ device: 'device-a' }),
    });
  ...
  it('requires a valid token to revoke anyone', async () => {
    ...
    const res = await SELF.fetch(`https://example.com/room/${room}/pair/revoke`, {
      method: 'POST',
      body: JSON.stringify({ device: 'device-a' }),
    });
    expect(res.status).toBe(401);
  ...
  it('rejects re-registering a revoked device id via a fresh pairing code', async () => {
    ...
    const { deviceToken: tokenB } = (await consumeRes.json()) as { deviceToken: string };
    ...
    const secondIssueRes = await SELF.fetch(`https://example.com/room/${room}/pair/issue`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${tokenB}` },
      body: JSON.stringify({ device: 'device-b' }),
    });
    const { code: secondCode } = (await secondIssueRes.json()) as { code: string };

    const reregisterRes = await SELF.fetch(`https://example.com/room/${room}/pair/consume`, {
      method: 'POST',
...
describe('inbox queue', () => {
  it('queues a message for an offline device and returns it on drain', async () => {
    ...
    a.send(
      JSON.stringify({ v: 1, room, device: 'device-a', to: 'device-b', kind: 'handoff', iv: 'x', ciphertext: 'y', ts: 1 }),
    );
    await new Promise((resolve) => setTimeout(resolve, 20));

    const drainRes = await SELF.fetch(`https://example.com/room/${room}/inbox/drain`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${tokenB}` },
      body: JSON.stringify({ device: 'device-b' }),
    });
    const { items } = (await drainRes.json()) as { items: Array<{ id: string; envelope: unknown }> };
  ...
  it('keeps a queued item until it is explicitly acked, not merely delivered', async () => {
    ...
    const firstDrain = await SELF.fetch(`https://example.com/room/${room}/inbox/drain`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${tokenB}` },
      body: JSON.stringify({ device: 'device-b' }),
    });
    const { items } = (await firstDrain.json()) as { items: Array<{ id: string }> };
    expect(items).toHaveLength(1);

    const secondDrain = await SELF.fetch(`https://example.com/room/${room}/inbox/drain`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${tokenB}` },
      body: JSON.stringify({ device: 'device-b' }),
    });
    expect(((await secondDrain.json()) as { items: unknown[] }).items).toHaveLength(1);
    ...
    const thirdDrain = await SELF.fetch(`https://example.com/room/${room}/inbox/drain`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${tokenB}` },
      body: JSON.stringify({ device: 'device-b' }),
    });
    expect(((await thirdDrain.json()) as { items: unknown[] }).items).toHaveLength(0);
  ...
  it('does not let device-c drain a message addressed to device-b', async () => {
    ...
    expect(((await drainRes.json()) as { items: unknown[] }).items).toHaveLength(0);
  ...
  it('queues a `to: all` broadcast for a registered device that is not currently connected', async () => {
    ...
    a.send(
      JSON.stringify({ v: 1, room, device: 'device-a', to: 'all', kind: 'presence', iv: 'x', ciphertext: 'y', ts: 1 }),
    );
    await new Promise((resolve) => setTimeout(resolve, 20));
    ...
    const { items } = (await drainRes.json()) as { items: Array<{ id: string; envelope: { to: string } }> };
    expect(items).toHaveLength(1);
...
describe('WebSocket upgrade auth', () => {
  it('rejects an upgrade with no token', async () => {
    ...
    const res = await SELF.fetch(`https://example.com/room/${room}?device=device-a`, {
      headers: { Upgrade: 'websocket' },
    });
    expect(res.status).toBe(401);
  });
  ...
  it('rejects an upgrade with a revoked token', async () => {
    ...
    const res = await SELF.fetch(`https://example.com/room/${room}?device=device-a&token=${deviceToken}`, {
      headers: { Upgrade: 'websocket' },
    });
    expect(res.status).toBe(401);
  });
...
describe('abuse limits', () => {
  ...
  it('caps queue depth per device', async () => {
    ...
    for (let i = 0; i < MAX_QUEUE_DEPTH_PER_DEVICE + 5; i++) {
      a.send(
        JSON.stringify({ v: 1, room, device: 'device-a', to: 'device-b', kind: 'handoff', iv: 'x', ciphertext: `y${i}`, ts: i }),
      );
    }
    await new Promise((resolve) => setTimeout(resolve, 50));
...
describe('CORS', () => {
  it('answers an OPTIONS preflight for an HTTP route', async () => {
    ...
    const res = await SELF.fetch(`https://example.com/room/${room}/pair/issue`, { method: 'OPTIONS' });
  ...
  it('sets Access-Control-Allow-Origin on an error response too', async () => {
    ...
    const res = await SELF.fetch(`https://example.com/room/${room}/inbox/drain`, {
      method: 'POST',
      body: JSON.stringify({ device: 'device-a' }),
    });
    expect(res.status).toBe(401);
