...
const HEADER = { room: 'room1', device: 'device-a', to: 'device-b', kind: 'handoff' as const };
...
describe('buildEnvelope / openEnvelope', () => {
  ...
  it('fails when the wrong key is used', async () => {
    ...
    const envelope = await buildEnvelope(key, HEADER, { url: 'https://example.com', title: 'x' });
  ...
  it('rejects a payload that decrypts but does not match the kind\'s shape', async () => {
    ...
    const envelope = await buildEnvelope(key, { ...HEADER, kind: 'stash' }, { not: 'a stash list' } as never);

    await expect(openEnvelope(key, envelope)).rejects.toThrow(InvalidPayloadError);
  });
});
